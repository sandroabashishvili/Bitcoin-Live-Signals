"""File: entry_location.py
Folder: platform_v2/tools/research/replay
Created date: 2026-05-25
Last updated date: 2026-05-25
Author: Codex
Purpose: Shared entry-location classifier for research replay tools.
"""
...
class ResearchEntryLocationClassifier:
    """Classify whether a historical entry was clean, extended, or near an extreme."""
    ...
    def classify(
        ...
        *,
        ...
        return {
            "type": cls._verdict(side=normalized_side, flags=flags),
            "flags": flags,
    ...
    def _verdict(*, side: str, flags: list[str]) -> str:
        if not flags:
            return "CLEAN"
        if side in {"LONG", "BUY"} and "UPPER_EXTENSION" in flags and "INTO_RESISTANCE" in flags:
            return "EXTENDED_INTO_RESISTANCE"
        if side in {"SHORT", "SELL"} and "LOWER_EXTENSION" in flags and "INTO_SUPPORT" in flags:
