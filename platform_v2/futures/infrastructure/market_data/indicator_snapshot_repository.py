"""Read futures indicator snapshots from runtime storage."""
...
from platform_v2.futures.storage import indicator_snapshot_file_name, indicator_snapshot_file_path, load_json_list
...
class JsonIndicatorSnapshotRepository:
    ...
    def get_snapshots(
        ...
        if self._indicator_root is None:
            source_path = indicator_snapshot_file_path(symbol, timeframe)
        else:
            source_path = self._indicator_root / symbol / indicator_snapshot_file_name(timeframe)
        raw_rows = (
