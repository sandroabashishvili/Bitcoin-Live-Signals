...
def build_portfolio_trade_outcome_items(metrics: dict[str, Any]) -> list[dict[str, str]]:
    return [
        ...
        metric_text_item("Profit Locks", metrics.get("profit_lock_hits", "—"), kind="tp"),
        ...
        metric_currency_item("Peak Capital", metrics.get("peak_capital"), kind="pnl"),
        metric_currency_item("Lowest Capital", metrics.get("lowest_capital"), kind="sl"),
        metric_text_item("LONG Trades", metrics.get("long_positions", "—"), kind="tp"),
        metric_text_item("SHORT Trades", metrics.get("short_positions", "—"), kind="sl"),
        metric_currency_item("LONG Net PnL", metrics.get("long_net_pnl"), kind="pnl"),
        metric_currency_item("SHORT Net PnL", metrics.get("short_net_pnl"), kind="pnl"),
    ]
...
def build_overview_strategy_snapshot_items(
    *,
    ...
    return [
        ...
        metric_text_item("LONG Signals", buy_signals, kind="tp"),
...
def build_strategy_activity_items(metrics: dict[str, Any]) -> list[dict[str, str]]:
    return [
        ...
        metric_text_item("LONG Signals", metrics.get("long_signals_since_start", metrics.get("buy_signals_since_start", "—")), kind="tp"),
        metric_text_item("SHORT Signals", metrics.get("short_signals_since_start", "—"), kind="sl"),
...
def build_strategy_activity_evaluation_items(summary: Mapping[str, Any]) -> list[dict[str, str]]:
    return [
        metric_text_item("Total Signals", summary.get("total_signals", "—")),
        metric_text_item("LONG Signals", summary.get("buy_signals", "—"), kind="tp"),
        metric_text_item("SHORT Signals", summary.get("short_signals", "—"), kind="sl"),
...
def build_strategy_funnel_items(metrics: dict[str, Any]) -> list[dict[str, str]]:
    return [
        metric_text_item("Total Signals", metrics.get("total_signals_since_start", "—")),
        metric_text_item("LONG Signals", metrics.get("long_signals_since_start", metrics.get("buy_signals_since_start", "—")), kind="tp"),
        metric_text_item("Denied Signals", metrics.get("denied_entries_since_start", "—"), kind="sl"),
        metric_text_item("Trades Opened", metrics.get("trades_opened_since_start", "—"), kind="neutral"),
        metric_text_item("Closed Trades", metrics.get("closed_positions", "—"), kind="neutral"),
    ]
