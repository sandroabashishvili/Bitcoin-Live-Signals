"""Backend-prepared content builders for the Portfolio page."""
...
class PortfolioPageContentService:
        ...
        *,
        ...
        return {
            "open_positions_section": {
                ...
                "intro": "Live positions with entry, size, TP/SL, and unrealized performance.",
    ...
    def _build_open_position_row(self, row: dict[str, Any]) -> list[str]:
        ...
        return [
            ...
            self._strong(row.get("side", "—")),
            self._strong(self._fmt_number(execution.get("entry_price"))),
            self._strong(self._fmt_usd(position_size)),
            ...
            self._strong(self._fmt_usd(row.get("unrealized_pnl")), kind="pnl"),
            self._strong(self._fmt_pct_from_notional(row.get("unrealized_pnl"), position_size), kind="pnl"),
        ]
    ...
    def _build_closed_position_row(self, row: dict[str, Any]) -> list[str]:
        ...
        return [
            self._escape(self._fmt_timestamp(row.get("opened_at"))),
            self._escape(self._fmt_timestamp(row.get("closed_at") or row.get("opened_at"))),
            self._escape(row.get("symbol", "—")),
            self._strong(row.get("side", "—")),
            ...
            self._strong(self._fmt_duration(row.get("opened_at"), row.get("closed_at"))),
            self._strong(self._fmt_number(execution.get("rr_ratio"))),
    ...
    def _build_open_position_detail_items(self, row: dict[str, Any]) -> list[dict[str, str]]:
        ...
        details = [
            ...
            ("Decision Time", row.get("decision_time"), "generic"),
            ...
            ("Size", self._fmt_usd(position_size), "generic"),
            ("Entry", self._fmt_number(execution.get("entry_price")), "generic"),
            ("Unrealized", self._fmt_usd(unrealized), "pnl"),
            ("Unreal %", self._fmt_pct_from_notional(unrealized, position_size), "pnl"),
    ...
    def _build_closed_position_detail_items(self, row: dict[str, Any]) -> list[dict[str, str]]:
        ...
        details = [
            ...
            ("Net %", self._fmt_pct_from_notional(net_pnl, position_size), "pnl"),
            ("TP", self._fmt_number(execution.get("take_profit")), "tp"),
            ("SL", self._fmt_number(execution.get("stop_loss")), "sl"),
            ...
            ("Opened", self._fmt_timestamp(row.get("opened_at")), "generic"),
    ...
    def _build_closed_trade_chart_row(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "duration": self._fmt_duration(row.get("opened_at"), row.get("closed_at")),
            "net_pct": self._fmt_pct_from_notional(net_pnl, position_size),
        }
    ...
    def _value_class(value: Any, *, kind: str = "generic") -> str:
        ...
        if kind == "sl":
            return "value-red"
        if text in {"BUY", "OPEN", "TP_HIT"}:
            return "value-green"
        if text in {"SL_HIT", "DENIED"}:
            return "value-red"
    ...
    def _fmt_timestamp(value: Any) -> str:
        ...
        if text.isdigit():
            try:
                return datetime.fromtimestamp(int(text) / 1000, UTC).strftime("%Y-%m-%d %H:%M:%S")
    ...
    @classmethod
    def _fmt_duration(cls, opened_at: Any, closed_at: Any) -> str:
        opened_dt = cls._parse_timestamp(opened_at)
        closed_dt = cls._parse_timestamp(closed_at)
        if opened_dt is None or closed_dt is None or closed_dt < opened_dt:
            return "—"
        total_seconds = int((closed_dt - opened_dt).total_seconds())
