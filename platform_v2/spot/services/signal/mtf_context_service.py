"""Independent Spot MTF rules, adapted from the Futures logic on 2026-09-10."""
...
class MultiTimeframeContextService:
    ...
    def build_signals(
        ...
        *,
        ...
        long_count = sum(1 for value in signals.values() if value == "BUY")
        short_count = sum(1 for value in signals.values() if value == "SELL")
        primary_is_long = signals.get(primary_timeframe) == "BUY"
        primary_is_short = signals.get(primary_timeframe) == "SELL"
        higher_timeframe_is_long = signals.get("4h") == "BUY"
        higher_timeframe_is_short = signals.get("4h") == "SELL"
