...
from platform_v2.spot.services.signal.long_structure_score_service import (
    SpotLongStructureScoreService,
)


class SpotLongComponentScoreService:
    ...
    def direction_mtf_score(context: MarketContext, *, direction: str) -> float:
        direction_text = {"LONG": "BUY", "SHORT": "SELL"}.get(direction.upper(), direction.upper())
    ...
    def long_orderbook_score(orderbook) -> float:
        if orderbook is None:
            return 0.0
        return SpotLongComponentScoreService.directional_orderbook_score(
    ...
    def long_structure_score(snapshot, price: float, atr_spike_block: bool) -> float:
        return SpotLongStructureScoreService.long_score(snapshot, price, atr_spike_block)

    @staticmethod
    def short_structure_score(snapshot, price: float, atr_spike_block: bool) -> float:
        return SpotLongStructureScoreService.short_score(snapshot, price, atr_spike_block)

    @staticmethod
    def distance_from_lower_atr(price: float, lower: float | None, atr: float | None) -> float | None:
        return SpotLongStructureScoreService.distance_from_lower_atr(price, lower, atr)

    @staticmethod
    def distance_to_lower_atr(price: float, lower: float | None, atr: float | None) -> float | None:
        return SpotLongStructureScoreService.distance_to_lower_atr(price, lower, atr)

    @staticmethod
    def distance_from_upper_atr(price: float, upper: float | None, atr: float | None) -> float | None:
        return SpotLongStructureScoreService.distance_from_upper_atr(price, upper, atr)

    @staticmethod
    def distance_to_upper_atr(price: float, upper: float | None, atr: float | None) -> float | None:
        return SpotLongStructureScoreService.distance_to_upper_atr(price, upper, atr)
